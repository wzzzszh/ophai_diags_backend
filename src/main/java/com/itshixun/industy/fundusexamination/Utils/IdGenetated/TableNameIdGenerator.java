...
public class TableNameIdGenerator implements IdentifierGenerator {
    ...
    public Serializable generate(SharedSessionContractImplementor session, Object object) {
        // 获取实体类名（默认表名）
        String entityName = object.getClass().getSimpleName().toUpperCase().substring(0, 2);
        // 构建 ID 组件
        String timestamp = String.valueOf(System.currentTimeMillis()).substring(3); // 最后 10 位时间戳
